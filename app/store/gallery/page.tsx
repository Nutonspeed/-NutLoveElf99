    ...
    <div className="min-h-screen flex flex-col">
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="flex flex-wrap gap-4 justify-center mb-6">
          ...
          <select value={color} onChange={e=>setColor(e.target.value)} className="border rounded p-1">
            <option value="all">All Colors</option>
            {colors.map(c=><option key={c} value={c}>{c}</option>)}
          </select>
          <select value={material} onChange={e=>setMaterial(e.target.value)} className="border rounded p-1">
      ...
      <ModalWrapper open={!!view} onClose={()=>setView(null)}>
        {view && (
          <div className="space-y-4">
            <Image src={view.image} alt={view.name} width={600} height={600} className="w-full h-auto" />
