...
export default function BillPage({ params }: { params: { id: string } }) {
  ...
  const admin = ref?.startsWith("chat_fb_") ? ref.replace("chat_fb_", "") : null

  const openChat = () => {
    try {
      const opened = window.open(chatwootUrl, "_blank")
      ...
      <div className="print:hidden bg-white border-b px-4 py-4">
        <div className="container mx-auto flex items-center justify-between">
          ...
          <div className="flex space-x-2">
            ...
            <Button variant="outline" onClick={openChat}>ย้อนกลับไปคุยกับแอดมิน</Button>
          </div>
        </div>
      </div>

      <div className="container mx-auto px-4 py-8 space-y-4 print:p-0">
        {admin && (
          <p className="text-center text-sm text-gray-600">
            ใบสรุปนี้จัดส่งโดยแอดมิน {admin}
          </p>
        )}
