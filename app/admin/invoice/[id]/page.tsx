...
import { Download, PrinterIcon as Print, ArrowLeft, Check, X, Mail } from "lucide-react"
...
export default function AdminInvoicePage({ params }: AdminInvoicePageProps) {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      if (data) {
        setOrder(data)
        setIsPaid(data.status === "delivered" || data.status === "shipped")
      }
      setLoading(false)
    }
    load()
  ...
  const handleMarkPaid = async () => {
    if (order) {
      await db.updateOrder(order.id, { status: order.status })
      setIsPaid(true)
    }
  }
  ...
  const handleResend = async () => {
    if (order?.customerEmail) {
      ...
      alert("ส่งใบเสร็จใหม่ทางอีเมลแล้ว")
    }
  }
      ...
      <div className="print:hidden bg-white border-b px-4 py-4">
        <div className="container mx-auto flex items-center justify-between">
          ...
          <div className="flex space-x-2">
            ...
            {isPaid ? (
              <Button variant="outline" onClick={handleMarkUnpaid}>
                <X className="mr-2 h-4 w-4" />
                ทำเครื่องหมายว่ายังไม่ชำระ
              </Button>
            ) : (
              ...
              <div className="grid grid-cols-2 gap-8">
                <div>
                  ...
                  <p className="text-sm text-gray-600">
                    สถานะ: {isPaid ? "ชำระเงินแล้ว" : "ยังไม่ชำระ"}
                  </p>
                </div>
                <div>
